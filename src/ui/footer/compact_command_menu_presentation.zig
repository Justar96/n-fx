...
test "compact status line menu renders toggled items without choose copy" {
    ...
    var tokens = try composeCompactCommandMenuRow(std.testing.allocator, projection, 4, rows, 80);
    defer tokens.deinit(std.testing.allocator);
    try std.testing.expect(std.mem.find(u8, tokens.items, "Tokens") != null);
    try std.testing.expect(std.mem.find(u8, tokens.items, "off") != null);
