...
test "status line menu describes toggle changes without performing effects" {
    ...
    try std.testing.expect(menu.move(1));
    const enable_tokens = menu.selectedChange(enabled).?;
    try std.testing.expectEqual(SettingId.statusline_tokens, enable_tokens.setting);
    try std.testing.expectEqualStrings("on", enable_tokens.value);
