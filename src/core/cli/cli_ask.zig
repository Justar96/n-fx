...
test "image preparation failure has stable text and JSON contracts" {
    ...
    const json = try renderErrorJsonResult(alloc, @errorName(error.ImagePreparationFailed), .result);
    defer alloc.free(json);
    try std.testing.expectEqualStrings(
        "{\"v\":1,\"output\":\"\",\"exit_code\":1,\"model\":\"\",\"session_id\":\"\",\"steps\":0,\"tool_calls\":[],\"error\":\"ImagePreparationFailed\",\"error_detail\":{\"code\":\"internal\",\"retryable\":false}}\n",
        json,
    );
}
