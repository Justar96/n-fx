...
pub const AutoUpgrade = struct {
    ...
    fn downloadAndInstall(
        ...
        const archive_url = helpers.releaseAssetUrl(alloc, cdn_base, target.artifactRef(), helpers.platform, false) catch return error.AllocFailed;
        ...
        const checksum_url = helpers.releaseAssetUrl(alloc, cdn_base, target.artifactRef(), helpers.platform, true) catch return error.AllocFailed;
        ...
        const extracted_bin = std.fmt.allocPrint(alloc, "{s}/nfx", .{tmp_dir}) catch return error.AllocFailed;
