...
fn failureMessage(err: RunError) []const u8 {
    return switch (err) {
        error.FetchFailed => "failed to fetch latest version from GitHub Releases",
...
fn upgradeWorkerInner(
    ...
    const archive_url = try helpers.releaseAssetUrl(alloc, cdn_base, target.artifactRef(), helpers.platform, false);
    ...
    const checksum_url = try helpers.releaseAssetUrl(alloc, cdn_base, target.artifactRef(), helpers.platform, true);
    ...
    const extracted_bin = try std.fmt.allocPrint(alloc, "{s}/nfx", .{tmp_dir});
